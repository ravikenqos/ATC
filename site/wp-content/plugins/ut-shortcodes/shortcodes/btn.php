...
if( !class_exists( 'UT_BTN' ) ) {
    ...
    class UT_BTN {
        ...
        function ut_create_shortcode( $atts, $content = NULL ) {
            ...
            $css_style = '<style type="text/css" scoped>';
				...
				if( !empty( $button_icon_size ) ) {
					$css_style .= '#' . $button_id . ' a i { line-height: inherit; }';
					$css_style .= '#' . $button_id . ' a i:before { font-size:' . $button_icon_size . 'px; vertical-align: bottom; }';					
				}
            ...
            if( !empty( $effect ) && $effect != 'none' ) {
				...
				if( $button_particle == 'yes' ) {
					...
					if( !empty( $animation_duration ) ) {
						$p_attributes['data-animation-duration'] = esc_attr( $animation_duration );    
					}                
		...
		function enqueue_scripts() {
            ...
            wp_print_scripts('ut-button-particles-js');

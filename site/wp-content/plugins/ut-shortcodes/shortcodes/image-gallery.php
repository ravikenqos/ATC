        ...
        function ut_create_inline_css( $id, $atts ) {
           ...
           $css = '<style type="text/css" scoped>';
				...
				if( $caption_background && ut_is_gradient( $caption_background ) ) {
					...
					$css .= ut_create_gradient_css( $caption_background, '#' . esc_attr( $id ) . ' .ut-image-gallery-item.ut-animation-done:hover .ut-image-gallery-image-caption', false, 'background' );
				...
				if( $caption_font_weight ) {
					$css .= '#' . esc_attr( $id ) . ' .ut-image-gallery-item-caption-title h3 { font-weight: '. $caption_font_weight . '; }';					
				}
                ...
                if( $shadow ) {
					...
					$css .= '#' . esc_attr( $id ) . ' .ut-image-gallery-image { margin-bottom: 20px; }';
                }
                ...
                $output .= '<div id="' . esc_attr( $id ) . '" class="ut-image-gallery ' . implode( ' ', $classes ) . ' clearfix">';
                    ...
                    foreach( $gallery as $image ) {
                        ...
                        if( $lightbox_size == 'hd' ) {
                            ...
                            $lightgallery = ut_resize( $lightgallery[0], 1280, 720, false, false, false );
                        ...
                        $output .= '<div ' . $attributes . ' data-count="' . $this->image_total_count . '" class="ut-image-gallery-item ' . implode( ' ', $animation_classes ) . ' grid-' . esc_attr( $desktop ) . ' tablet-grid-' . esc_attr( $tablet ) . ' mobile-grid-100 ' . implode( ' ', $image_classes ) . '">';
                            ...
                            if( $external_link == 'yes' && $attachment_project_url ) {
                                ...
                                $output .= '<a ' . $title . ' href="' . esc_url( $attachment_project_url ) . '" target="_' . $external_link_target . '">';

                            }
                        	
							if( $shadow ) {
								$shadow = 'gutter-shadow';
							}
						
                            $output .= '<div class="ut-image-gallery-image ' . $shadow . '">';
